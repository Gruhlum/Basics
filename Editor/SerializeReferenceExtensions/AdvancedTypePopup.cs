...
namespace MackySoft.SerializeReferenceExtensions.Editor
{
    ...
    public class AdvancedTypePopup : AdvancedDropdown
    {
        ...
        public static void AddTo(AdvancedDropdownItem root, IEnumerable<Type> types)
        {
            ...
            // Add null item.
            AdvancedTypePopupItem nullItem = new AdvancedTypePopupItem(null, TypeMenuUtility.k_NullDisplayName)
            {
                id = itemCount++
            };
            ...
            foreach (Type type in typeArray)
            {
                ...
                if (!isSingleNamespace)
                {
                    for (int k = 0; (splittedTypePath.Length - 1) > k; k++)
                    {
                        ...
                        {
                            AdvancedDropdownItem newItem = new AdvancedDropdownItem(splittedTypePath[k])
                            {
                                id = itemCount++,
                            };
